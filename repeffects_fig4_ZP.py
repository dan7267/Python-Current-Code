...
def produce_basic_statistics(y, plag):
    ...
    AMS = calculate_AMS(y)
    AMA = calculate_AMA(y)
    CPPM = calculate_CPPM(y)
    CPZP = calculate_CPZP(y)

    return AM, CP, WC, BC, AMA, AMS, CPPM, CPZP
...
def calculate_CPPM(y):
    ...
    for sub in range(n_subs):
        ...
        Fm = np.mean(F, axis=1)
        n_trials_p1 = cond1_p1.shape[0]
        Fm_p1 = Fm[:n_trials_p1]
        Fm_p2 = Fm[n_trials_p1:]
        ...
        tmp = np.corrcoef(A)
        wc1 = tmp[np.triu_indices_from(tmp, k=1)]
        bwc.append(np.mean(wc1))

        tmp = np.corrcoef(B)
        wc2 = tmp[np.triu_indices_from(tmp, k=1)]
        bwc.append(np.mean(wc2))

        wc = np.mean(np.concatenate([wc1, wc2]))
        bwc = np.mean(bwc)
        ...
        bc_matrix = tmp[:nA, nA:]
        bc = np.mean(bc_matrix[np.triu_indices(min(nA, nB), k=1)])

        cp1 = wc - bc
        cp1_array[sub] = cp1
    print(cp1_array)
    return np.mean(cp1_array)

def calculate_CPZP(y):
    #y is 18 x 32 x 200
    n_subs, n, v = y.shape
    cp1_array = np.zeros((n_subs, 1))
    for sub in range(n_subs):
        ...
        n_trials_p1 = cond1_p1.shape[0]
        Fz_p1 = Fz[:n_trials_p1, :]
        Fz_p2 = Fz[n_trials_p1, :]
        A = Fz_p1.T
        B = Fz_p2.T
        bwc = []

        # Correlation of A with itself
        tmp = np.corrcoef(A)
        wc1 = tmp[np.triu_indices_from(tmp, k=1)]
        bwc.append(np.mean(wc1))

        # Correlation of B with itself
        tmp = np.corrcoef(B)
        wc2 = tmp[np.triu_indices_from(tmp, k=1)]
        bwc.append(np.mean(wc2))
        ...
        # Step 6: Compute between-class correlation
        tmp = np.corrcoef(A, B)
        nA = A.shape[1]
        nB = B.shape[1]
        bc_matrix = tmp[:nA, nA:]
        ...
        # Step 7: Classification performance = WC - BC
        cp1 = wc - bc
        cp1_array[sub] = cp1
    print(cp1_array)
    return np.mean(cp1_array)
...
def produce_slopes(y, pflag):
    ...
    AM, CP, WC, BC, AMA, AMS, CPPM, CPZP = produce_basic_statistics(y, pflag)
